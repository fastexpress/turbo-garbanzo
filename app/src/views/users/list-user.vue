          ...
          <div class="custom-table">
            ...
            >
              <template #cell(rol)="row">
                <b-badge variant="secondary">{{ row.item.rol.name }}</b-badge>
              </template>
...
export default {
  metaInfo: { title: "Lista de usuarios" },
  data() {
    return {
      breadcrumb: [
        { text: "Inicio", href: "/" },
  ...
  methods: {
    ...
    get() {
      ...
      axios
        .get(
          this.api() +
            `/users?show=${this.show}&search=${this.search}&page=${this.pagination.current_page}`
        )
        .then((response) => {
          this.isBusy = false;
