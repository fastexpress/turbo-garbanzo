        ...
        <div class="panel br-6 p-0">
          <div class="custom-table">
            <div class="table-header">
              ...
              <b-form-group
                :label="
                  admin === true ? 'Mostrar mi cuenta' : 'Todas las cuentas'
                "
              >
                <label class="switch s-icons s-outline s-outline-primary">
                  <input type="checkbox" v-model="admin" @change="get" />
          ...
          <div class="custom-table">
            ...
            >
              ...
              <template #cell(amount)="row">
                {{ dollarOrQuetzal(row.item) }}
              </template>
              <template #cell(in_charge)="row">
                <b-badge variant="info">{{ row.item.in_charge.name }}</b-badge>
              </template>
...
export default {
  ...
  methods: {
    ...
    dollarOrQuetzal(item) {
      if (item.sign === "$") {
        return this.myDollar(item.amount);
      } else {
        return this.myMoney(item.amount);
      }
    },
    myMoney(value) {
    ...
    delete(id) {
      ...
      axios
        .delete(this.api() + `/cashregister/${id}`)
        .then((response) => {
          loader.hide();
    ...
    get() {
      ...
      axios
        .get(
          this.api() +
            `/cashregister?show=${this.show}&search=${this.search}&page=${this.pagination.current_page}&user=${this.userID()}&admin=${this.admin}`
        )
        .then((response) => {
          this.isBusy = false;
