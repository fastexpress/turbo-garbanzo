...
export default {
  ...
  methods: {
    ...
    find() {
      ...
      axios
        .get(this.api() + `/package-ups/${this.$route.params.id}`)
        .then((response) => {
          loader.hide();
          const { packageUPS, customers, boxes, content } = response.data;
          this.id = packageUPS.id;
          this.guide = packageUPS.guide;
          this.observation = packageUPS.observation;
          this.receive = packageUPS.receive;
          this.sender = packageUPS.sender;
          this.telephone = packageUPS.telephone;
          this.alternativeTelephone = packageUPS.alternativeTelephone;
          this.typical = packageUPS.typical === 1 ? true : false;
          if (this.typical === true) {
            this.$refs["typical"].address = packageUPS.address;
            this.$refs["typical"].postalCode = packageUPS.postalCode;
            this.$refs["typical"].state = packageUPS.state;
            this.$refs["typical"].city = packageUPS.city;
            this.$refs["typical"].show = false;
            this.$refs["typical"].customer = customers.map((c) => {
              return {
                id: c.customers[0].id,
                name: c.customers[0].name,
                basePrice: c.customers[0].basePrice,
                multiplicater: c.customers[0].multiplicater,
                exchangeRate: c.customers[0].exchangeRate,
                type: c.customers[0].type === 1 ? true : false,
                credit: c.customers[0].credit,
                prices: JSON.parse(c.customers[0].prices),
                weight: c.weight,
                subtotal: c.subtotal,
                balance: c.balance,
                cost: c.cost,
                updatedMultiplier: c.updatedMultiplier,
                checkCredit: c.typePaymentTypical === 1 ? true : false,
                accountPersonal: {
                  id: c.accounts[0].id,
                  name: c.accounts[0].name,
                  amountBank: c.accounts[0].amountBank,
                  bank: c.accounts[0].bank,
                },
              };
            });
          } else {
            this.$refs["customer"].departament = JSON.parse(
              packageUPS.departament
            );
            this.$refs["customer"].type = JSON.parse(packageUPS.type);
            this.$refs["customer"].category = JSON.parse(packageUPS.category);
            this.$refs["customer"].typePayment =
              packageUPS.typePayment === 1 ? true : false;
            this.$refs["customer"].weight = packageUPS.weight;
            this.$refs["customer"].cost = packageUPS.cost;
            this.$refs["customer"].multiplier = packageUPS.multiplicater;
            this.$refs["customer"].updatedMultiplier =
              packageUPS.updatedMultiplier === 1 ? true : false;
            this.$refs["customer"].office_guatemala = packageUPS.office;
            // this.$refs["customer"].total = packageUPS.totalPayment;

            this.$refs["customer"].address = packageUPS.address;
            this.$refs["customer"].postalCode = packageUPS.postalCode;
            this.$refs["customer"].state = packageUPS.state;
            this.$refs["customer"].city = packageUPS.city;
            this.$refs["customer"].show = false;
          }
          this.inOffice = this.$moment(packageUPS.inOffice).toDate();
          this.baler = {
            id: packageUPS.baler[0].id,
            name: packageUPS.baler[0].name,
            code: packageUPS.baler[0].code,
          };
          this.boxes = boxes;
    ...
    handleSubmit() {
        ...
        data = {
          ...
          state:
            this.$refs["typical"].show === false
              ...
              : typeof State.getStateByCodeAndCountry(
                  this.$refs["typical"].state,
                  this.$refs["typical"].country
                ) === "undefined"
              ? this.$refs["typical"].state
      ...
      axios
        .put(this.api() + `/package-ups/${this.id}`, data)
        .then((response) => {
    ...
    deleteBox(id) {
      const index = this.boxes.findIndex((x) => String(x.id) === String(id));
      if (index !== -1) {
