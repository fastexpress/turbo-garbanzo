          ...
          <div class="custom-table">
            ...
            >
              ...
              <template #cell(status)="row">
                <b-badge
                  variant="warning"
                  v-if="row.item.status === 'GENERATED'"
                  >PENDIENTE DE CÓDIGO UPS</b-badge
                >
              </template>
  ...
  methods: {
    ...
    get(customer) {
      ...
      axios
        .get(
          this.api() +
            `/search-package-ups-pendent?show=${this.show}&search=${this.search}&page=${this.pagination.current_page}&customer=${customer}`
        )
        .then((response) => {
          this.isBusy = false;
    ...
    downloadListPackages() {
      window.open(
        this.api() + `/package-ups-download?customer=${this.customer}`,
        "_blank"
      );
    },
  },
