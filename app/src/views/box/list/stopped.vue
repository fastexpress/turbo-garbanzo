<template>
  <div>
        ...
        <div class="panel br-6 p-0">
          <div class="custom-table">
            <div class="table-header">
              ...
              <download
                :typeReport="{ name: 'Reclamo', value: 'STOPPED' }"
                :customer="customer"
              />
            </div>
          </div>
        ...
        <div class="panel br-6 p-0">
          <div class="custom-table">
            ...
            >
              ...
              <template #cell(inStoppedComment)="row">
                <b-badge variant="danger">{{
                  row.item.inStoppedComment
                }}</b-badge>
              </template>
    ...
    <continue-package ref="continuePackage" @save="$emit('call')" />
...
import continuePackage from "./modals/continue.vue";
...
export default {
  components: {
    "continue-package": continuePackage,
    ups,
    download,
  },
  data() {
    return {
      ...
      columns: [
        ...
        { key: "inStoppedComment", label: "Estado" },
  ...
  methods: {
    ...
    openModalUPS(itemPackage) {
      this.$refs["ups"].openModal(itemPackage);
    },
    openModal(itemPackage) {
      this.$refs["continuePackage"].openModal(itemPackage);
    },
    myLbs(value) {
    ...
    get(customer) {
      ...
      axios
        .get(
          this.api() +
            `/search-package-ups-stopped?show=${this.show}&search=${this.search}&page=${this.pagination.current_page}&customer=${customer}`
        )
        .then((response) => {
          this.isBusy = false;
