        ...
        <div class="panel br-6 p-0">
          <div class="custom-table">
            <div class="table-header">
              ...
              <download
                :typeReport="{ name: 'Listo para envío', value: 'READY' }"
                :customer="customer"
              />
            </div>
          </div>
          ...
          <div class="custom-table">
            ...
            >
              ...
              <template #cell(status)="row">
                <b-badge variant="warning" v-if="row.item.status === 'READY'"
                  >LISTO PARA ENVIÓ</b-badge
                >
              </template>
...
export default {
  ...
  methods: {
    ...
    ifAccept(id) {
      this.$swal({
        title: "Quieres cambiar este paquete como en ruta?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
    ...
    accept(id) {
      let loader = this.$loading.show();
      axios
        .post(this.api() + `/package-ups-route`, {
          id: id,
          user: this.userID(),
        })
        .then((response) => {
    ...
    get(customer) {
      ...
      axios
        .get(
          this.api() +
            `/search-package-ups-ready?show=${this.show}&search=${this.search}&page=${this.pagination.current_page}&customer=${customer}`
        )
        .then((response) => {
          this.isBusy = false;
