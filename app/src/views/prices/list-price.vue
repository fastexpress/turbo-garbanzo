        ...
        <div class="panel br-6 p-0">
          <div class="custom-table">
            ...
            >
              ...
              <template #cell(departaments)="row">
                <b-badge
                  variant="secondary"
                  v-for="departament in row.item.departaments"
                  :key="departament.id"
                  >{{ departament.name }}</b-badge
                >
              </template>
              <template #cell(basePrice)="row">
                {{ myDollar(row.item.basePrice) }}
              </template>
              <template #cell(multiplicater)="row">
                {{ myDollar(row.item.multiplicater) }}
              </template>
...
export default {
  metaInfo: { title: "Lista de precios" },
  data() {
    return {
      breadcrumb: [
        { text: "Inicio", href: "/" },
      ...
      columns: [
        { key: "id", label: "ID" },
        { key: "isDelivery", label: "Entrega" },
        { key: "categories", label: "Categorias" },
        { key: "departaments", label: "Departamentos" },
        { key: "basePrice", label: "Precio base" },
        { key: "multiplicater", label: "Multiplicador" },
  ...
  methods: {
    ...
    get() {
      ...
      axios
        .get(
          this.api() +
            `/prices?show=${this.show}&search=${this.search}&page=${this.pagination.current_page}`
        )
        .then((response) => {
          this.isBusy = false;
