...
export default {
  metaInfo: { title: "Lista de empacadoras" },
  data() {
    return {
      breadcrumb: [
        { text: "Inicio", href: "/" },
        { text: "Lista de empacadoras", active: true },
      ],
      items: [],
      columns: [
        { key: "id", label: "ID" },
  ...
  methods: {
    ...
    get() {
      ...
      axios
        .get(
          this.api() +
            `/balers?show=${this.show}&search=${this.search}&page=${this.pagination.current_page}`
        )
        .then((response) => {
          this.isBusy = false;
