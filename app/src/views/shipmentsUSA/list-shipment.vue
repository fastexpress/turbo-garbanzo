        ...
        <div class="panel br-6 p-0">
          <div class="custom-table">
            ...
            >
              ...
              <template #cell(bags)="row">
                <b-badge
                  variant="success"
                  v-for="bag in row.item.bags"
                  :key="bag.id"
                  >{{ bag.bag }} | {{ myLbs(bag.capacity) }}</b-badge
                >
              </template>
...
export default {
  metaInfo: { title: "Lista de envios" },
  data() {
    return {
      breadcrumb: [
        { text: "Inicio", href: "/" },
        { text: "Lista de envios", active: true },
      ],
      items: [],
      columns: [
        { key: "id", label: "ID" },
  ...
  methods: {
    ...
    delete(id) {
      ...
      axios
        .delete(this.api() + `/shipments/usa/${id}`)
        .then((response) => {
          loader.hide();
    ...
    get() {
      ...
      axios
        .get(
          this.api() +
            `/shipments/usa?show=${this.show}&search=${this.search}&page=${this.pagination.current_page}`
        )
        .then((response) => {
