      ...
      <div class="panel br-6 p-0">
        <div class="custom-table">
          ...
          >
            ...
            <template #cell(callStatus)="row">
              <b-badge variant="warning" v-if="row.item.callStatus === 0"
                >Pendiente</b-badge
              >
              <b-badge variant="success" v-if="row.item.callStatus === 1"
                >Llamado</b-badge
              >
              <b-badge variant="danger" v-if="row.item.callStatus === 2"
                >No contesto</b-badge
              >
            </template>
            ...
            <template #cell(action)="row">
              <div class="position-relative">
                ...
                >
                  ...
                  <b-dropdown-item
                    @click="
                      addObservation(row.item.id, row.item.observationCall)
                    "
                    link-class="action-edit"
                  >
                    <svg
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: "callStatus", label: "Llamada" },
        { key: "status", label: "Lugar" },
        { key: "packages", label: "Paquetes" },
        { key: "observationCall", label: "Observación" },
  ...
  methods: {
    ...
    get(date, city) {
      ...
      axios
        .get(
          this.api() +
            `/search-package-call?show=${this.show}&search=${
              this.search
            }&page=${
